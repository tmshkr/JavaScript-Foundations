...
*/
let monthlyMortgagePayment = (P * (I * (1 + I) ** N)) / ((1 + I) ** N - 1);
...
function mortgageCalculator(principal, interestRate, years, creditScore) {
  ...
  const N = years * 12;
  const monthlyPayment = (P * (I * (1 + I) ** N)) / ((1 + I) ** N - 1);
  return monthlyPayment.toFixed(2);
}
console.log(mortgageCalculator(200000, 3, 30));
...
function variableInterestRate(principal, interestRate, years, name) {
  interestRate -= 2;
  for (let i = 0; i < 9; i++) {
    const monthlyPayment = mortgageCalculator(principal, interestRate, years);
    console.log(
...
variableInterestRate(200000, 4, 30, "Bob");
// 🌟🌟🌟 STRETCH 🌟🌟🌟//
